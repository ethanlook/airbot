...
func NewMoveManager(dependencies resource.Dependencies, airBot *AirBot, logger golog.Logger) (Move, error) {
	ms, err := motion.FromDependencies(dependencies, "builtin")
	if err != nil {
		return nil, err
	}
	slam, err := resource.FromDependencies[slam.Service](dependencies, resource.NewName(slam.API, airBot.Config.SlamService))
	if err != nil {
		return nil, err
	}
	base, err := base.FromDependencies(dependencies, airBot.Config.BaseComponent)
	if err != nil {
		return nil, err
	}
